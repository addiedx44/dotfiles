...
# try to update the motd, or else add it to the bottom of the .bash_profile
if [[ ! `uname -s` = 'Darwin' && -f /etc/update-motd.d ]]; then
  ...
  if [[ `/usr/bin/id -u` -ne 0 ]]; then
    ...
    if [[ ! `command -v ponysay` ]]; then
      ...
      if command -v cowsay >/dev/null 2>&1 && command -v fortune >/dev/null 2>&1; then
        # need these because we're running as root and root doesn't have /usr/games in his path
        ...
        echo "#!/bin/bash" | tee /etc/update-motd.d/51-fortune-pony >/dev/null 2>&1 &
